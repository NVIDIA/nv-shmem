...
public:
  SensorMapTests() {
    ...
    mShmem = std::make_unique<Map<SensorMap, SensorValue>>(name_space, O_CREAT,
                                                           1024 * 1000);
  }
...
TEST_F(SensorMapTests, testSensorMapCreate) {
  ...
  for (int i = 0; i < 5; i++) {
    ...
    nv::shmem::SensorValue value(std::to_string(i),
...
TEST_F(SensorMapTests, testSensorMapReadOnlyMapErrorInUpdate) {
  auto name_space = "maptest";
  auto mShmemROnly =
      std::make_unique<Map<SensorMap, SensorValue>>(name_space, O_RDONLY);
  auto sensorName = "HGX_Chassis_0_My_Sensor_1";
  nv::shmem::SensorValue value(std::to_string(100),
                               "/redfish/v1/HGX_Chassis_0/Sensors/Sensor_1",
                               1699255438, "1/1/2022");
